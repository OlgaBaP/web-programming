# Olga Baturo
![Profile Photo](photo_me.jpg)
## About Me

I am a student at the BRU, majoring in software engineering.  
I enjoy learning new technologies and continuously improving my skills.  

My goal is to become a professional software developer.

"If you're gonna burn a bridge, you better hold your breath and learn to swim."

## Skills

- C# programming  
- Windows Forms application development  
- HTML and CSS (actively learning)  
- Basic knowledge of computer hardware and electronic devices  

## Code Example

class Program
{
    static void Main()
    {
        Console.WriteLine("Hello, World!");
    }
}

## Work Experience

### Curriculum Vitae

A personal CV website created using pure HTML.

More about my project: View Project

## Courses and Trainings

- C# Programming Course  
- Fundamentals of cryptographic information protection (Stepik)  
- Introduction to programming in C++ (Stepik)  
- Computer systems and peripheral devices  

## English Level

My English level is B1.

1. Reading  
2. Writing  
3. Speaking  

## Contacts

Phone: +375 33 350 00 19 
Email: olgapir241@gmail.com  
Telegram: @yuanfe_nn  
